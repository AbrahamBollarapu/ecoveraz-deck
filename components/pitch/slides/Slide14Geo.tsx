...
const HUBS: Hub[] = [
  { city: "Delaware", region: "USA", purpose: ["Corporate HQ", "Investor + governance readiness", "US market entry"] },
  { city: "Berlin", region: "Germany", purpose: ["R&D + architecture", "EU CSRD alignment", "Standards + defensibility"] },
  { city: "Hyderabad", region: "India", purpose: ["Operations + deployments", "Integration delivery", "Cost-efficient scale"] },
];

export function Slide14Geo({ mode }: { mode: DemoMode }) {
  const chipLabel = mode === "regulator" ? "REGULATOR CONTEXT" : "INVESTOR CONTEXT";

  return (
    <div className="h-full w-full min-w-0">
      {/* MOBILE: simple list + why it matters */}
      <div className="block md:hidden space-y-4">
        <Card title="Global execution footprint" desc="Each location exists for delivery and defensibility.">
          <div className="mt-4 grid gap-3">
            {HUBS.map((h) => (
              <div key={h.city} className="rounded-2xl border border-line bg-bg p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold">{h.city}, {h.region}</div>
                    <div className="mt-1 text-xs font-mono text-sub">Execution hub</div>
                  </div>
                  <div className="h-8 w-8 rounded-xl border border-line bg-surface flex items-center justify-center">
        ...
        <Card title="Why it matters" desc="Footprint supports adoption and audit confidence.">
          <div className="mt-4 space-y-3">
            <Tile tone="bg-bg">
              <div className="text-sm font-semibold">Regulatory proximity</div>
              <div className="mt-2 text-sm text-sub leading-relaxed">EU and US readiness reduces adoption friction and improves audit confidence.</div>
            </Tile>
            <Tile tone="bg-bg">
              <div className="text-sm font-semibold">R&D defensibility</div>
              <div className="mt-2 text-sm text-sub leading-relaxed">Standards-aligned engineering strengthens verification claims.</div>
            </Tile>
            <Tile tone="bg-bg">
              <div className="text-sm font-semibold">Operational scale</div>
              <div className="mt-2 text-sm text-sub leading-relaxed">Deployments and integration playbooks become repeatable.</div>
            </Tile>
          </div>
        </Card>
      </div>

      {/* TABLET/DESKTOP: keep your map panel layout */}
      <div className="hidden md:grid gap-4 md:grid-cols-12 h-full min-w-0">
          ...
          <div className="mt-4 rounded-2xl border border-line bg-bg p-4 h-[420px] relative overflow-hidden">
            ...
            <div className="absolute inset-0 p-4">
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <TeamChipRow items={[{ tone: "neutral", label: "PLACEHOLDER" }, { tone: mode === "regulator" ? "good" : "neutral", label: chipLabel }]} />
              </div>
        ...
        <Card className="md:col-span-4 h-full min-w-0" title="Why it matters" desc="Footprint supports adoption and audit confidence.">
          <div className="mt-4 space-y-3">
            <Tile tone="bg-bg">
              <div className="text-sm font-semibold">Regulatory proximity</div>
              <div className="mt-2 text-sm text-sub leading-relaxed">EU and US readiness reduces adoption friction and improves audit confidence.</div>
